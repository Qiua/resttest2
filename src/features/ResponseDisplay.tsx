...
export const ResponseDisplay: React.FC<ResponseDisplayProps> = ({ response, loading, error }) => {
  if (loading) {
    return <div className='p-4 text-gray-500 dark:text-gray-400'>Carregando...</div>
  }
  ...
  if (!response) {
    return <div className='p-4 text-gray-500 dark:text-gray-400'>A resposta da sua requisição aparecerá aqui.</div>
  }
  ...
  const responseTabs = [
    {
      ...
      content: isJson ? (
        ...
        <pre className='p-4 rounded-md bg-gray-900 dark:bg-gray-800 text-white dark:text-gray-100 text-sm h-full overflow-auto'>
    ...
    },
    {
      label: 'Headers',
      content: (
        <pre className='p-2 rounded-md bg-gray-900 dark:bg-gray-800 text-white dark:text-gray-100 text-sm'>
  ...
  return (
    <div className='space-y-4 h-full flex flex-col'>
      <div className='flex-shrink-0'>
        <h3 className='text-lg font-medium text-gray-700 dark:text-gray-300 mb-2'>Status da Resposta</h3>
